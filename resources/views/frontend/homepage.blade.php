...
@section('content')
    ...
    <section class="hero" id="hero" aria-label="Introduction">
        ...
        <div class="hero-inner">
            ...
            <div class="hero-photo-wrap">
                ...
                <img src="{{ isset($profileImage) ? asset('profile-images/' . $profileImage) : asset('img/profile.jpg') }}"
                    alt="{{ $profileName ?? 'Md. Sadikuzzaman' }}" class="hero-photo">
                    ...
                    <div class="social-links" style="margin-top:16px; display:flex; gap:12px; flex-wrap:wrap;">
                        @if ($linkedin)
                            <a href="{{ $linkedin }}" target="_blank" rel="noopener noreferrer"
                                class="badge bg-secondary text-white">LinkedIn</a>
                        @endif
                        @if ($facebook)
                            <a href="{{ $facebook }}" target="_blank" rel="noopener noreferrer"
                                class="badge bg-secondary text-white">Facebook</a>
                        @endif
                        @if ($twitter)
                            <a href="{{ $twitter }}" target="_blank" rel="noopener noreferrer"
                                class="badge bg-secondary text-white">Twitter</a>
                        @endif
                        @if ($instagram)
                            <a href="{{ $instagram }}" target="_blank" rel="noopener noreferrer"
                                class="badge bg-secondary text-white">Instagram</a>
                        @endif
                        @if ($wikipedia)
                            <a href="{{ $wikipedia }}" target="_blank" rel="noopener noreferrer"
                                class="badge bg-secondary text-white">Wikipedia</a>
                        @endif
                    </div>
                @endif
    ...
    @include('frontend.partials.artist-profile-preview')
    ...
    <div class="container">
        <div class="phd-highlight reveal">
            ...
            <div class="phd-content">
                ...
                <p>School of Design Thinking Graduate School &bull; Mode: Conventional (Thesis-Based) &bull; Duration:
                    3&ndash;5 Years &bull; Advancing research in educational leadership, design thinking, and institutional
                    innovation in South and Southeast Asian contexts.</p>
            </div>
        </div>
    </div>
    ...
    <div class="container">
        ...
        <section class="section" id="career" aria-labelledby="career-title">
            ...
            <div class="timeline-v2">
                <div class="tl-row reveal">
                    ...
                    <div class="tl-center">
                        <div class="tl-dot2 a"></div>
                        <div class="tl-line"></div>
                    </div>
                    <div class="tl-card">
                        <div class="role">Principal</div>
                        <div class="place">Daulatpur College, Daulatpur, Kushtia . Appointed 29 November 2017. Joined: 03
                            December 2017</div>
                    </div>
                </div>
        ...
        @if (isset($blogs) && count($blogs) > 0)
            <section class="section" id="latest-news" aria-labelledby="latest-news-title">
                ...
                <div class="blog-grid"
                    style="display:grid; grid-template-columns:repeat(auto-fit,minmax(260px,1fr)); gap:20px; margin-top:16px;">
                    @php
                        $homeBlogs =
                            $blogs instanceof \Illuminate\Contracts\Pagination\Paginator
                                ? collect($blogs->items())
                                : collect($blogs);
                    ...
                    @foreach ($homeRecent as $blog)
                        <div class="blog-card"
                            style="border-radius:14px; overflow:hidden; box-shadow:0 10px 30px rgba(0,0,0,0.06); background:#fff;">
                            @if (!empty($blog->image_url))
                                <img src="{{ $blog->image_url }}" alt="{{ $blog->{'blog-title'} }}"
                                    style="width:100%; height:180px; object-fit:cover; display:block;">
                            @else
                                <div
                                    style="width:100%; height:180px; background:#f4f0e8; display:flex; align-items:center; justify-content:center; color:#9a8a70;">
                                    No image</div>
                            @endif
                            <div style="padding:16px; display:flex; flex-direction:column; gap:10px;">
                                <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
                                    <span
                                        style="background:#fff4dd; color:#8d6527; padding:6px 10px; border-radius:999px; font-weight:700; border:1px solid rgba(201,163,90,0.25); font-size:12px;">
                                    ...
                                    @if ($blog->publication_name)
                                        <span
                                            style="background:#fff4dd; color:#8d6527; padding:6px 10px; border-radius:999px; font-weight:700; border:1px solid rgba(201,163,90,0.25); font-size:12px;">{{ $blog->publication_name }}</span>
                                    @endif
                                </div>
                                <h3 class="home-blog-title" style="margin:0; font-size:1.05rem;">
                                    {{ \Illuminate\Support\Str::limit($blog->{'blog-title'}, 80) }}</h3>
                                <p style="margin:0; color:#555; line-height:1.6;">
                                    {{ \Illuminate\Support\Str::limit($blog->subtitle ?? ($blog->description ?? ''), 120) }}
                                </p>
                                <div style="margin-top:auto; display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
                                    <button type="button" class="blog-read-btn"
                                        style="background:#c9a35a; color:#fff; padding:10px 14px; border-radius:999px; text-decoration:none; font-weight:700; border:none; cursor:pointer;"
                                        ...
                                        data-article-url="{{ e($blog->article_url ?? '') }}">Read Preview</button>
                                    @if ($blog->article_url)
                                        <a href="{{ $blog->article_url }}" target="_blank" rel="noopener noreferrer"
                                            style="background:#e8d5b7; color:#555; padding:8px 12px; border-radius:999px; text-decoration:none; font-weight:700;">View
                                            Source</a>
                                    @endif
                                    <a href="{{ url('/blog') }}"
                                        style="background:transparent; color:#c9a35a; padding:8px 12px; border-radius:999px; text-decoration:none; font-weight:700; border:1px solid #c9a35a;">More</a>
                                </div>
                            </div>
                        </div>
        ...
        <section class="section" id="leadership" aria-labelledby="leadership-title">
            ...
            <div class="pos-grid" id="pos-grid">
                <div class="pos-card">
                    <h4>Daulatpur Pilot Model Secondary School</h4>
                    <p>Chairman : 2009&ndash;2016</p>
                </div>
                <div class="pos-card">
                    <h4>Rifayetpur Secondary School</h4>
                    <p>Chairman : 2014&ndash;2015</p>
                </div>
                <div class="pos-card">
                    <h4>Goalgram College</h4>
                    <p>Chairman : 2014&ndash;2016 &amp; Jan 2024&ndash;Sep 2024</p>
                </div>
                <div class="pos-card">
                    <h4>Pragpur College</h4>
                    <p>Chairman : 2014&ndash;September 2024</p>
                </div>
                <div class="pos-card">
                    <h4>Daulatpur Dakhil Madrasa</h4>
                    <p>Chairman : 2014&ndash;2016</p>
                </div>
                <div class="pos-card">
                    <h4>Khalishakundi Degree College</h4>
                    <p>Chairman : 2021&ndash;September 2024</p>
                </div>
                <div class="pos-card">
                    <h4>National University, Bangladesh</h4>
                    <p>Senate Member : 2018&ndash;2020 &amp; 2024&ndash;Sep 2024</p>
                </div>
                <div class="pos-card">
                    <h4>Daulatpur Central Graveyard Committee</h4>
                    <p>President : 2020&ndash;2024</p>
                </div>
            </div>
        </section>
            ...
            <div class="edu-list">
                <div class="edu-item reveal">
                    ...
                    <div class="edu-body">
                        ...
                        <p>GenoVasi University College, Malaysia &bull; Design Thinking Graduate School &bull; Research
                            (Thesis) Mode</p>
                    </div>
                </div>
        ...
        <section class="section" id="publication" aria-labelledby="publication-title">
            ...
            <div class="pub-card reveal">
                ...
                <div class="pub-body">
                    ...
                    <p>Published by the Asiatic Society of Bangladesh. Contributed comprehensive documentation on the
                        Liberation War history of Daulatpur, providing rare archival information on the region&apos;s role
                        in the 1971 War of Independence.</p>
                </div>
            </div>
        </section>
                ...
                <div class="gallery reveal">
                    @foreach ($galleryItems as $item)
                        <div class="gallery-item" style="transition-delay: {{ $loop->index * 0.1 }}s">
                            ...
                            <div class="gallery-overlay"><span class="gallery-overlay-text">{{ $item->title }}</span>
                            </div>
                ...
                <div class="row">
                    @foreach ($journals as $j)
                        <div class="col-md-6 mb-4">
                            <div class="card">
                                @if ($j->image)
                                    <img src="{{ asset('journal-images/' . $j->image) }}" class="card-img-top"
                                        alt="">
                                @endif
                                <div class="card-body">
                                    <h5 class="card-title"><a
                                            href="{{ url('/journals/' . $j->slug) }}">{{ $j->title }}</a></h5>
                                    <p class="card-text"><strong>{{ $j->authors }}</strong> — {{ $j->journal_name }}
                                    </p>
    ...
    <div class="quote-section" aria-label="Featured quote">
        <div class="quote-inner reveal">
            ...
            <blockquote>Education is the most powerful weapon which you can use to change the world &mdash; and every
                institution I lead is a step toward a brighter Bangladesh.</blockquote>
                                ...
                                @foreach ($social_medias as $s)
                                    @if (!empty($s['url']))
                                        <a href="{{ $s['url'] }}" target="_blank" rel="noopener noreferrer"
                                            class="badge bg-secondary text-white">{{ $s['name'] ?? 'Link' }}</a>
                                    @endif
                                @endforeach
                            @endif
                        </div>
    ...
    <aside id="blogSlidePanel" class="blog-slide-panel" aria-hidden="true" aria-labelledby="blog-slide-title">
        <div class="blog-slide-header">
            <div>
                ...
                <p style="margin: 6px 0 0; color: #6f6f6f; font-size: 0.95rem;">Story summary and publication details in
                    one place.</p>
            </div>
            <button type="button" id="blogSlideClose" class="blog-slide-close"
                aria-label="Close blog preview">×</button>
        </div>
        <div class="blog-slide-content">
            ...
            <div class="blog-slide-footer">
                <a id="blogSlideSourceLink" href="#" target="_blank" rel="noopener noreferrer">Open source
                    article</a>
            </div>
        </div>
    </aside>
